...
from optimization_function_multiple_sites import (
    optimize_static_config,
    extract_site_params,
)
...
MONGO_URI = os.getenv(
    "MONGO_URI",
    "mongodb+srv://rmaskey:f9Rdxe4TXDfiZkkI@cluster0.8ytdjij.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0",
)
...
@app.route("/optimize", methods=["POST"])
def optimize():
    """Optimize site configurations based on forecasts"""
    try:
        ...
        # Hash price forecast
        h = (
            pd.read_csv("../datasets/forecasts/hash_forecast.csv")
            .iloc[:, 1]
            .tolist()[:T]
        )
        ...
        # Token price forecast
        g = (
            pd.read_csv("../datasets/forecasts/token_forecast.csv")
            .iloc[:, 1]
            .tolist()[:T]
        )
        ...
        site_index = {
            site["id"]: idx for idx, site in enumerate(sites_data) if "id" in site
        }
...
def debug_site_structure():
    ...
    try:
        ...
        site.pop("_id", None)
        return jsonify({"fields": list(site.keys()), "sample": site}), 200
    except Exception as e:
